...
public class ColorChanger : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        if(colorRed >= 1f && colorGreen <= 0.1f && colorBlue <= 0.1f)
        {
            curColorChange = "greenUp";
        }

        if (colorRed >= 1f && colorGreen >= 1f && colorBlue <= 0.1f)
        {
            curColorChange = "redDown";
        }

        if (colorRed <= 0.1f && colorGreen >= 1f && colorBlue <= 0.1f)
        {
            curColorChange = "blueUp";
        }

        if (colorRed <= 0.1f && colorGreen >= 1f && colorBlue >= 1f)
        {
            curColorChange = "greenDown";
        }

        if (colorRed <= 0.1f && colorGreen <= 0.1f && colorBlue >= 1f)
        {
            curColorChange = "redUp";
        }

        if (colorRed >= 1f && colorGreen <= 0.1f && colorBlue >= 1f)
        {
